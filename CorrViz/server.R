shinyServer(function(input, output) {
  ...
  output$CorViz <- renderPlot({
    ...
    gene  <- fetchAbundance(cohortFile, studyID, type = "Relative", entity = "Genes", index = list(input$GeneSelect,NULL))
  ...
  output$CorBars <- renderPlot({
    plotData <- fetchCorrelations(cohortFile, studyID, rows = "Genes", columns = "Genera", index = list(input$GeneSelect, NULL))
    boxplot(plotData)
    })
})